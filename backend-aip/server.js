...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ roomId }) => {
    ...
    activeGames.set(roomId, {
      roomId,
      players: Object.values(room.players).map(p => ({ color: p.color })),
      spectators: 0,
      startTime: Date.now()
    });
    
    // Broadcast updated active games list
    io.emit('activeGamesUpdate', Array.from(activeGames.values()));
  ...
  socket.on('gameStarted', ({ roomId }) => {
    ...
    // Add to active games that can be watched
    activeGames.set(roomId, {
      roomId,
      players: Object.values(room.players).map(p => ({ color: p.color })),
      spectators: 0,
    ...
    // Broadcast updated active games list to everyone
    io.emit('activeGamesUpdate', Array.from(activeGames.values()));
  });
  ...
  socket.on('joinAsSpectator', ({ roomId }) => {
    ...
    if (activeGames.has(roomId)) {
      const gameInfo = activeGames.get(roomId);
      gameInfo.spectators++;
      activeGames.set(roomId, gameInfo);
    ...
    io.to(roomId).emit('spectatorJoined', { spectatorCount: activeGames.get(roomId)?.spectators || 1 });
    
    console.log(`Spectator joined room: ${roomId}`);
  });

  socket.on('getActiveGames', () => {
    socket.emit('activeGamesUpdate', Array.from(activeGames.values()));
  });
  ...
  socket.on('readyToRestart', ({ roomId }) => {
    ...
    if (room.players[socket.id]) {
      ...
      if (allPlayers.length === 2 && allPlayers.every(id => readyPlayers.includes(id))) {
        ...
        // Re-add to active games if it was removed
        if (!activeGames.has(roomId)) {
          activeGames.set(roomId, {
            roomId,
            players: Object.values(room.players).map(p => ({ color: p.color })),
  ...
  socket.on('disconnect', () => {
    ...
    // Handle spectators leaving
    for (const [roomId, gameInfo] of activeGames.entries()) {
      if (socket.rooms && socket.rooms.has(roomId)) {
        // This was a spectator
        gameInfo.spectators = Math.max(0, gameInfo.spectators - 1);
        if (gameInfo.spectators === 0 && !rooms.has(roomId)) {
          // No spectators and no game, remove from active games
          activeGames.delete(roomId);
        } else {
          activeGames.set(roomId, gameInfo);
        }
        ...
        io.to(roomId).emit('spectatorLeft', { spectatorCount: gameInfo.spectators });
      }
    }
    ...
    for (const [roomId, room] of rooms.entries()) {
      if (room.players[socket.id]) {
        ...
        if (Object.keys(room.players).length === 0) {
          ...
          // Remove from active games when a player disconnects
          if (activeGames.has(roomId)) {
            activeGames.delete(roomId);
          }
        }
